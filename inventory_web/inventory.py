...
def inventory_items(page: int = 1, page_size: int = 100) -> tuple[list[dict[str, str | None]], int, int, int]:
    ...
    with _connection() as connection:
        ...
        rows = connection.execute(
            'SELECT item_no, item_name, unit_no, barcode FROM "库存" ORDER BY item_no LIMIT ? OFFSET ?',
...
def search_inventory(query: str, page: int = 1, page_size: int = 100) -> tuple[list[dict[str, str | None]], int, int, int]:
    """按条码、商品编码或商品名称模糊查询本地库存表。"""
    query = query.strip()
    if not query:
        raise ValueError("请输入条码、商品编码或商品名称。")
    page, page_size = _page(page, page_size)
        ...
        total = connection.execute(
            '''SELECT COUNT(*) FROM "库存"
               WHERE barcode LIKE ? ESCAPE '\\'
                  OR item_no LIKE ? ESCAPE '\\'
                  OR item_name LIKE ? ESCAPE '\\' ''',
            (pattern, pattern, pattern),
        ).fetchone()[0]
        rows = connection.execute(
            '''SELECT i.item_no, i.item_name, i.unit_no, i.barcode, ic.category_id
               FROM "库存" i
               LEFT JOIN item_categories ic ON ic.item_no = i.item_no
               WHERE i.barcode LIKE ? ESCAPE '\\'
                  OR i.item_no LIKE ? ESCAPE '\\'
...
def sync_inventory() -> int:
    """读取外部商品主数据，并以完整快照覆盖本地库存表。"""
    try:
        with get_connection() as connection:
            cursor = connection.cursor()
            cursor.execute("""
                SELECT item_no, item_name, unit_no, barcode
    ...
    try:
        with _connection() as connection:
            ...
            connection.executemany(
                'INSERT INTO "库存" (item_no, item_name, unit_no, barcode) VALUES (?, ?, ?, ?)', rows
            )
    except Exception as error:
